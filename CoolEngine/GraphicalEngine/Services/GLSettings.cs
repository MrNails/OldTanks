...
public readonly struct GLSettings
{
    ...
    public static void RestoreGLSettings(in GLSettings settings)
    {
        ...
        if (settings.BlendIsActive) GL.Enable(EnableCap.Blend);
        else GL.Disable(EnableCap.Blend);
        
        if (settings.ScissorIsActive) GL.Enable(EnableCap.ScissorTest);
        else GL.Disable(EnableCap.ScissorTest);
        
        if (settings.CullFaceIsActive) GL.Enable(EnableCap.CullFace);
        else GL.Disable(EnableCap.CullFace);
        
        if (settings.DepthTestIsActive) GL.Enable(EnableCap.DepthTest);
        else GL.Disable(EnableCap.DepthTest);
        
        if (settings.StencilTestIsActive) GL.Enable(EnableCap.StencilTest);
